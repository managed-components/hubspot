...
export const sendEvent = (settings: ComponentSettings) => async (event: MCEvent) => {
  const { accountId, regionPrefix, domainName } = settings;
  const { client, payload, type } = event;
  const { identifyEmail, identifyUserId, n, property_name, property_value, ...cleanPayload } = payload;
  ...
  const hubspotParams = {
    ...
    sd: client.screenHeight && client.screenWidth 
  ...
  client.fetch(`https://track${getRegionPrefix(regionPrefix)}.hubspot.com/__ptq.gif?${params}`, {
    credentials: 'include',
    keepalive: true,
    mode: 'no-cors'
  });
...
export const handleChatEvent = (settings: ComponentSettings) => async ({ client }: MCEvent) => {
  const { accountId, regionPrefix } = settings;
  client.execute(`!function(t,e,r){if(!document.getElementById(t)){var n=document.createElement("script");for(var a in n.src="https://js${getRegionPrefix(regionPrefix)}.usemessages.com/conversations-embed.js",n.type="text/javascript",n.id=t,r)r.hasOwnProperty(a)&&n.setAttribute(a,r[a]);var i=document.getElementsByTagName("script")[0];i.parentNode.insertBefore(n,i)}}("hubspot-messages-loader",0,{"data-loader":"hs-scriptloader","data-hsjs-portal":${accountId},"data-hsjs-env":"prod"${ regionPrefix ? `,"data-hsjs-hublet":"${regionPrefix}"` : '' }});`);
}
...
export const handleFormEvent = (settings: ComponentSettings) => async (event: MCEvent) => {
  const { accountId, regionPrefix } = settings;
  event.client.fetch(`https://forms${getRegionPrefix(regionPrefix)}.hubspot.com/collected-forms/submit/form`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(
      getFormEventRequestData(event, accountId)
    )
  })
}
